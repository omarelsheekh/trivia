...
class TriviaTestCase(unittest.TestCase):
    ...
    def test_delete_question(self):
        ...
        resp=self.client.delete('/api/questions/{}'.format(self.test_question.id)).json
        self.assertTrue(resp['success'])

    def test_post_question(self):
        resp=self.client.post('/api/questions/',json={
            'question':'POST_QUSTION','answer':'','category':1,'difficulty':1
        }).json
        self.assertTrue(resp['success'])

    def search_question(self):
        self.test_question.insert()
        resp=self.client.post('/api/search/questions',json={'searchTerm':'test'}).json
        self.assertTrue(resp['success'])
